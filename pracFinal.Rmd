---
title: "Fundamentos  matemáticos  del  análisis  de  datos - Práctica Final"
...
```
 ...
 El set de datos a tratar contiene información de clientes de un Proveedor de servicois de telecomunicaciones tales como telefonía, Internet...
  
 ![Raw Data](img/info_basica.png)  
...
# Primer análisis de las variables
 ...
 El primer paso realizado anterior al análisis de las variables ha sido el filtrado de éstas. Se ha eliminado la columna 'customerID' dado que cada valor es único para cada cliente y no tiene ningún valor. Además se filtrarán aquellas filas en las que falten valores, para tener los datos con la mejor estructura posible. Además, vamos a aplicar la función *as.factor()* en las columnas de valor discreto para trabajar con éstas.
...
```
 ...
 ![Hist vbles continuas](img/hist_continuas.png)  

 ![Barchar vbles discretas](img/barchart_discretas1.png)  
  
 ![Barchar vbles discretas](img/barchart_discretas2.png)  

 Para conocer la relación entre variables se ha representado de forma gráfica las correlaciones entre ellas:

 ![Matriz Corr](img/corr.png)  
...
```
 ...
 Mirando solo el diagrama de dispersion se ve que están bastante relacionadas, dado que la línea representada no es horizontal. También podemos conocer de forma más 'analítica' si ambas variables están relacionadas utilizando el **Test Chi Cuadrado**.
 
```{r}
chisq.test(fdata$tenure, fdata$MonthlyCharges)
```

  Como el p-Valor es muy pequeño por lo que se rechaza H0, siendo esta que son independientes. Hay relacion entre la permanencia de un cliente y lo que paga al mes; en concreto, cuantos más meses está un cliente, más paga al mes, lo que puede significar que los clientes tienden o a mantener sus contratos (aunque sean altos) dado que los servicios les convencen, o que los van aumentando cuanto más tiempo lleven en la compañía.
...
ggplot(fibra_1anho, aes(x = Partner, y = MonthlyCharges)) +
  ...
  scale_y_continuous(name = "MonthlyCharges") +
  ggtitle("Gasto mensual de los clientes según si han abandonado la compañía") + 
  theme_minimal()
```
...
mosaicplot(Partner ~ Dependents, data = fibra_1anho_high,color = "skyblue2",
           border = "chocolate")
#addmargins(table(fibra_1anho$Contract, fibra_1anho$gender))
...
```
  ...
  * P(abandono|2-años): 2.8%
  ...
  La compañía parece tener un grave problema con los contratos *Mes-a-mes*, dado que casi la mitad de clientes abandonan la compañía. Aún así también es interesante conocer si esos clientes que abandonan al acabar el mes es porque se aprovechan de que la duración es muy corta o simplemente tienen ese contrato, pero se van tras un **largo tiempo** (tenure) con los servicios contratados.
...
## Comparativa de clientes mediante tests de hipótesis
 ...
 Por otro lado, la compañía también estudia no seguir contratando el equipo de servicio técnico que ahora mismo, puesto que consideran que en caso de que los clientes con este servicio no puedan asegurar una permanencia alta no les saldría rentable el coste de contratación del equipo. 
...
```
 ...
 Vemos que a medida de que aumenta el gasto mensual, aumenta el número de meses que se mantiene un cliente en la compañía. Aún así, no se puede poner el precio extremadamente alto porque supondría una grandísima barrera de entrada a potenciales clientes que podría acabar con la empresa.
